...
[![Rust](https://img.shields.io/badge/Rust-runtime-black?logo=rust)](https://www.rust-lang.org/)
[![Python](https://img.shields.io/badge/Python-legacy%20runtime-blue?logo=python)](https://www.python.org/)
...
CC Desktop Switch 是一个面向 **Claude Desktop 官方桌面客户端** 的轻量配置工具。它用桌面界面管理 DeepSeek、Kimi、智谱 GLM、阿里云百炼、小米 MiMo 等 API 提供商，并一键写入 Claude Desktop 的第三方推理配置。

和 `farion1231/cc-switch` 这类偏 Claude Code / CLI 的工具不同，本项目的定位是：让普通用户在 Windows 和 macOS 上更方便地配置 Claude Desktop 官方客户端。

v1.1.0 起，桌面运行时迁移到 **Tauri + Rust**，保留原有 HTML/CSS/JavaScript 前端界面和既有 Python 兼容路径。Windows 安装版、便携版和 macOS App 默认会打开独立桌面窗口；浏览器地址只作为调试和备用入口。关闭窗口时应用会隐藏到托盘或 Dock 后台继续运行，单击托盘或重新打开应用可以唤回窗口。需要完全退出时，请使用托盘、Dock 或系统菜单里的退出入口。
...
- `CC-Desktop-Switch-v<版本>-Windows-Setup.exe`：Windows 当前用户安装版，默认安装到用户目录，会创建桌面和开始菜单快捷方式，不需要管理员权限。
- `CC-Desktop-Switch-v<版本>-Windows-Portable.zip`：Windows 便携版，解压后直接运行，不写入安装记录，也不会自动创建快捷方式。
- `CC-Desktop-Switch-v<版本>-Windows-x64.exe`：Windows 单文件便携版，适合临时运行，不会自动创建快捷方式。
...
- 在 Provider 启用后提示是否立即重启 Claude Desktop，并在 Claude 未运行时直接打开。
- 提供应用内反馈入口，可带脱敏诊断信息提交问题。
...
5. 按提示点击“立即重启”或手动完整重启 Claude Desktop 后测试。
...
CC Desktop Switch is a lightweight desktop app for the official Claude Desktop client. It helps Windows and macOS users configure third-party Anthropic-compatible API providers such as DeepSeek, Kimi, Zhipu GLM, Xiaomi MiMo, and Alibaba Cloud Bailian, including Token Plan presets.
...
- The v1.1.0 desktop runtime uses Tauri + Rust while keeping the existing provider-management UI and local Python compatibility path available during the migration.
