...
v1.1.0 起，桌面运行时迁移到 **Tauri + Rust**，保留原有 HTML/CSS/JavaScript 前端界面。Windows 安装版、便携版和 macOS App 默认会打开独立桌面窗口。关闭窗口时应用会隐藏到托盘或 Dock 后台继续运行，单击托盘或重新打开应用可以唤回窗口。需要完全退出时，请使用托盘、Dock 或系统菜单里的退出入口。
...
普通 Anthropic 兼容供应商默认直连，不依赖本机转发端口。只有实验兼容接口会用到本机转发端口：
