...
Windows 推荐使用 `Windows-Setup.exe` 安装版。它默认安装到当前用户目录，会创建桌面和开始菜单快捷方式，不需要管理员权限。便携 zip 解压后也可以直接运行，但不会自动创建快捷方式。
...
完成后可以在弹出的提醒里点击“立即重启”，也可以稍后手动完整重启 Claude Desktop。重启后再正常发消息即可。
...
- macOS 版关闭窗口后会隐藏到后台，需要完全退出时请使用系统菜单、Dock 或应用托盘/菜单里的退出入口。
