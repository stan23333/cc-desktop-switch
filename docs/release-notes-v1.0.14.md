...
本次版本继续收口桌面端配置体验，并同步修复 macOS 应用内更新链路。
...
- 修复 macOS 应用内更新链路：下载更新时现在会显示明确反馈，安装流程会等待当前应用退出后再打开安装包，解决“下载无反馈”和“应用内安装失败”的问题。
...
This release continues to tighten the desktop configuration flow and fixes the in-app update flow on macOS.
...
- Fixed the macOS in-app update flow: the UI now shows clear download feedback, and the installer waits for the current app process to exit before launching, which resolves the missing feedback and failed in-app install issues.
